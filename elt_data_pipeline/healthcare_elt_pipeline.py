...
S3_BUCKET = "YOUR_S3_DATA_BUCKET"  # Replace with your actual airflow data S3 bucket
...
with DAG(
    ...
    run_doctors_raw_load = SQLExecuteQueryOperator(
        ...
        FORCE = TRUE;
        """ # Update the names of the database, schema, and table as needed
    )
    ...
    [run_doctors_raw_load, run_hospitals_raw_load, run_patients_raw_load, run_treatments_raw_load, run_visitis_raw_load] >> raw_loads_complete # Dummy task to ensure all raw loads complete before moving on
    ...
    [run_patients_transform_sp, run_treatments_transform_sp, run_visits_transform_sp] >> transform_load_complete # Dummy task to ensure all transform loads complete before moving on

    # Continue to Gold Layer SPs
    transform_load_complete >> [run_city_health_access_agg_sp,run_hospital_performance_agg_sp,run_patient_value_agg_sp,run_patient_visit_summary_agg_sp] >> agg_load_complete # Dummy task to ensure all agg loads complete before finishing the DAG